...
def main(save_sample, save_categories):
    ...
    with open (path2buisness, 'r') as f:
        for line in f.readlines():
            ...
            if r in bjc:
                ...
                if len(bjc) > 1:
                    # in this task we only need to work with one cuilsine
                    if cuisine not in bjc: continue
                    # keep a set with the restaurants for this cuisine
                    cuisineRestaurants.append(business_json)
    ...
    for sample_number in range(len(ratingFilter)):
        ...
        sample_cat2filteredReviewRestId={}
        ...
        if save_sample:  
            ...
            sample_filteredReviewReplicatedByCat_RestId = []
            ...
            for cat in sample_cat2filteredReviewText:
                ...
                while count < sample_size and sorted_rev_sample[count] < new_max_bound:
                    ind = sorted_rev_sample[count] - max_bound
                    sample_filteredReviewReplicatedByCat_Text.append(sample_cat2filteredReviewText[cat][ind].replace("\n", " ").strip())
                    sample_filteredReviewReplicatedByCat_Rating.append( sample_cat2filteredReviewRating[cat][ind] )
                    sample_filteredReviewReplicatedByCat_RestId.append( sample_cat2filteredReviewRestId[cat][ind] )
                    count = count + 1
            ...
            print("output map Category-to-RestId (randomly sampled reviews)")
            with open (dirpath + "review_restId_sample.txt", 'w') as f:
                f.write('\n'.join(sample_filteredReviewReplicatedByCat_RestId).encode('ascii','ignore') )
        ...
        if save_categories:
            ...
            # text corpus enhanced with rating and restaurant id
            for cat in sample_cat2filteredReviewText:
                if num_rev_cat[cat] < 1000: continue
                reviews = []
                for i in range(len(sample_cat2filteredReviewText[cat])):
                    row = "%s\t%s\t%s" % (sample_cat2filteredReviewText[cat][i], sample_cat2filteredReviewRating[cat][i], sample_cat2filteredReviewRestId[cat][i])
                    reviews.append(row)
                with open (path2categories + cat.replace('/', '-').replace(" ", "_") + "_review.txt" , 'w') as f:
                    f.write(u'\n'.join(reviews).encode('utf-8').strip())

            # save a subset of the business for this cuisine
            with open(dirpath + 'indianRestaurants.json', 'w') as outfile:
                json.dump(cuisineRestaurants, outfile)
