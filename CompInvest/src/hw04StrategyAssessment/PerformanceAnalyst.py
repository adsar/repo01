...
def analize_results(benchmark_symbol, l_event_strategies, start_date, end_date):
    for es in l_event_strategies:
        es.bench_sym = benchmark_symbol
 
        # 1 - Read the dates
        (start_date, end_date) = read_daterange(es.getValuesFile()) 
        
        # 2 - Read the data
        df_price = read_market_history([es.bench_sym], start_date, end_date) 
        ts_ref_price = df_price[es.bench_sym]
        ...
        # 5 - write output
        save_report(start_date, end_date, d_fund, d_ref, es.bench_sym, es.getReportFile(), es.i_transaction_count) 
        print_results(start_date, end_date, d_fund, d_ref, es.bench_sym, es.getName(), es.i_transaction_count) 
        plot_results(ts_value.index, d_fund["day_ret"], d_ref["day_ret"], es.bench_sym, es.getAnalysisFile())
