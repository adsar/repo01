...
twitter  = LOAD '/user/cloudera/pig/2/input/Twitter_Test_Samp200.txt' USING JsonLoader('created_time:chararray, text:chararray, user_id:chararray, id:chararray, created_date:chararray');
twt_samp = SAMPLE twitter 0.1;

-- ******************************************
-- get word counts for 1 date
-- ******************************************
-- The data contains only 2 dates here, Sun Sep 30 2012 and Mon Oct 08 2012
...
-- Save the data
STORE twt_d1dall_normed into '/user/cloudera/pig/2/twt_d1dall_normed';
...
-- hdfs dfs -copyToLocal /user/cloudera/pig/2/twt_d1dall_normed
